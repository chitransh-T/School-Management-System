...
const FeesCollectionPage: React.FC<{ params: Promise<{ studentId: string }> }> = ({ params }) => {
  ...
  //     setTimeout(() => router.push('/receiptgenerate'), 2000);
  ...
  const submitFees = async () => {
    ...
    try {
      ...
      const receiptData = {
        ...
        studentSection: studentSection || 'N/A',
        feeMonths: isYearlyPayment ? ['Yearly'] : selectedMonths,
        totalPaid: newTotalPaid,
        totalDue: Math.max(0, totalYearlyFee - newTotalPaid),
      ...
      setSuccess('Fee payment recorded successfully');
      setTimeout(() => router.push(`/receiptgenerate?data=${encodedData}`), 2000);
