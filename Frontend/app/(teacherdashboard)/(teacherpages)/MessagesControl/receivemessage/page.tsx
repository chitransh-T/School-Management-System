...
const ReceiveMessagesPage: React.FC = () => {
  ...
  const fetchMessages = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${baseUrl}/api/teacher/messages`, {
        headers: {
  ...
  const handleDeleteMessage = async (messageId: number) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${baseUrl}/api/received-messages/${messageId}`, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {messages.map((msg, index) => (
              <div
              key={msg.id || `msg-${index}`} // fallback to index if id is missing
              className="relative bg-blue-400 p-4 rounded-lg border border-white border-opacity-20"
            >
                <div>
                  <div className="text-white text-sm">
                    Message received from parents of {msg.student_name || "Unknown Student"}
                  </div>
